...
```mermaid
flowchart TD
    A["Why budgets matter"] --> B["Where money comes from"]
    B --> C["How budgets are structured"]
    C --> D["How the budget cycle works"]
    D --> E["How funds become services"]
    E --> F["How to read and evaluate results"]
    F --> G["How to participate"]
    G --> H["Follow-the-money capstone"]
```
