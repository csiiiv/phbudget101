...
3. **Editorial neutrality (hard rule):** learner-facing content names no advocacy organizations, coalitions, campaigns, fellowships, or personalities. Named entities are limited to government institutions, laws, documents, and public programs. Where the guidebook teaches through a named campaign or individual, reuse the *method* and the *publicly verifiable facts* (enacted budget items, dates, institutional actions), anonymized or generalized ("a transport coalition" is not allowed either — use "civil society advocacy" or an illustrative composite). Attribution to source organizations lives in the reference section's sources page, not in lessons.
