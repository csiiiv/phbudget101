...
* **Time estimates:** each lesson carries a reading-time estimate at authoring (guideline: 8–15 minutes per lesson; worked-example and interactive lessons up to 25). Modules display total estimated time; Module 0's "what this course covers" presents the full-course estimate (~3–4 hours, Modules 0–9; the capstone is optional) and quick-path estimate (~1–2 hours). Estimates are metadata, not narrative content, so they can be recomputed when lessons change.
