...
export function TraceConcern() {
  ...
  const concern = useMemo(() => CONCERNS.find((c) => c.id === concernId), [concernId]);
      ...
      <Card className="not-prose">
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {CONCERNS.map((c) => (
              <Button key={c.id} variant="outline" onClick={() => { setConcernId(c.id); setStepIndex(0); }}>
                {c.label}
              </Button>
            ))}
          </div>
      ...
      <CardContent className="space-y-4">
        <div className="space-y-1.5">
          <div className="text-xs uppercase tracking-wide text-muted-foreground">
            Step {stepIndex + 1} of {concern.steps.length}
          </div>
        ...
        <div className={cn('flex gap-2 no-print')}>
          ...
          {!isLast ? (
            ...
            <Button variant="secondary" onClick={() => { setConcernId(null); setStepIndex(0); }}>
              Start over
            </Button>
          )}
        </div>
        <p className="text-xs text-muted-foreground">
          Illustrative example. Details are simplified for clarity.
        </p>
      </CardContent>
    </Card>
  );
